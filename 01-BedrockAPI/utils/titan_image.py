...
def image_parameters():
    ...
    numberOfImages = st.slider('numberOfImages', value=1, min_value=1, max_value=5, step=1)
    params = {
        "cfgScale": cfgScale,
        "seed": seed,
        "quality": quality,
...
def get_titan_response_image(response):
    ...
    for image in images:
        ...
        io_image_data = BytesIO(image_data)
        list_images.append(io_image_data)
...
def get_image_from_model(model, prompt_content, negative_prompt=None, **params):
    ...
    body = get_titan_image_generation_request_body(
        prompt=prompt_content, negative_prompt=negative_prompt, **params)
