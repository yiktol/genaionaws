...
with text:
    st.title('Prompt Injection in LLMs')
    st.markdown("""This adversarial prompt example aims to demonstrate prompt injection where the LLM is originally \
...
essentially overriding the expected model behavior.""")
    ...
    with st.form('form1'):
        prompt = st.text_area(":orange[Prompt]", prompt, height=100)
